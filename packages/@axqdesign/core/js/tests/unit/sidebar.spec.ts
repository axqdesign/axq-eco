...
describe('sidebar folded toggle', () => {
  ...
  afterEach(() => {
    ...
    localStorage.removeItem('axqdesign-sidebar')
  })
  ...
  it('folds the sidebar to the hover variant on first click', () => {
    ...
    expect(localStorage.getItem('axqdesign-sidebar')).toBe('folded-hover')
  ...
  it('unfolds the sidebar on second click', () => {
    ...
    expect(localStorage.getItem('axqdesign-sidebar')).toBe('default')
  ...
  it('dispatches axqdesign:sidebar-folded with the new state', () => {
    const listener = vi.fn()
    document.addEventListener('axqdesign:sidebar-folded', listener, { once: true })
  ...
  it('ignores clicks outside a toggle', () => {
    ...
    expect(document.documentElement.hasAttribute('data-bs-sidebar')).toBe(false)
    expect(localStorage.getItem('axqdesign-sidebar')).toBeNull()
  })
})
