...
// The folded state may come pre-render from localStorage (axqdesign-theme.js), so
...
document.addEventListener('click', (event: MouseEvent) => {
  ...
  localStorage.setItem('axqdesign-sidebar', willFold ? 'folded-hover' : 'default')
  ...
  document.dispatchEvent(new CustomEvent('axqdesign:sidebar-folded', { detail: { folded: willFold } }))
})
