...
//
// Custom properties are authored bare in scss, so a name AXQ Design owns and a name
...
// is simply gone.
//
// axqdesign-vendors.scss is where every foreign name lives, so its full set of
...
describe('css custom-property prefixing', () => {
  it('leaves vendor-owned names alone and prefixes everything else', async () => {
    await expect(buildCustomPropertyNames('axqdesign-vendors.scss')).resolves.toMatchSnapshot()
  })
