...
export async function generateFont(strokeName, type, DIR) {
  ...
  const fileName = `axqdesign-icons${type === 'outline' ? (strokeName !== "400" ? `-${strokeName}` : '') : `-${type}`}`;
  ...
  const options = {
     name: `AXQ Icons ${type.charAt(0).toUpperCase() + type.slice(1)}`,
     fileName,
     glyphs,
