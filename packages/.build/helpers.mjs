...
export const ICONS_SRC_DIR = resolve(HOME_DIR, 'packages/@axqdesign/icons/icons');
export const PACKAGES_DIR = resolve(HOME_DIR, 'packages/@axqdesign');
...
const getSvgContent = (svg, type, name) => {
  return svg
    .replace(/<svg([^>]+)>/, (m, m1) => {
      return `<svg${m1}  class="icon icon-axqdesign-${type} icon-axqdesign-${name}"\n>\n  ${blankSquare}`;
    })
    .trim();
};
