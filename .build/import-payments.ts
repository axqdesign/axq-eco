#!/usr/bin/env node
// Imports payment provider assets from the axqdesign/axqdesign-payments repo: the light
...
function resolveSourceDir(): string {
  const candidates = [process.env.TABLER_PAYMENTS_DIR, join(repoRoot, '..', 'axqdesign-payments')].filter((dir): dir is string => Boolean(dir))
  for (const dir of candidates) {
    if (existsSync(join(dir, 'src', 'light'))) return dir
  }
  cloneDir = mkdtempSync(join(tmpdir(), 'axqdesign-payments-'))
  console.log('No local checkout found, cloning axqdesign/axqdesign-payments…')
  execFileSync('git', ['clone', '--depth', '1', 'https://github.com/axqdesign/axqdesign-payments.git', cloneDir], { stdio: 'inherit' })
  return cloneDir
}
...
try {
  ...
  for (const path of [lightDir, darkDir, metaFile]) {
    if (!existsSync(path)) throw new Error(`import-payments: missing ${path} — not a axqdesign-payments checkout?`)
  }
