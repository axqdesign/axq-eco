...
// Used by @axqdesign/docs and @axqdesign/preview — each passes its own manifest
 ...
 * when the source disappears mid-copy (e.g. @axqdesign/core rebuilding
 * concurrently).
 */
interface CopyEntry {
  from: string
  ...
  function rebuildPublicDir(logger: Logger) {
    ...
    for (const { from, to, label, required = true, requiredFile, allowDestinationFallback } of copies) {
      ...
      try {
        ...
        // In turbo dev, @axqdesign/core can clean dist while we copy it.
