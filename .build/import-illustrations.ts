#!/usr/bin/env node
// Imports illustrations from the axqdesign/axqdesign-illustrations repo: the light and
...
function resolveSourceDir(): string {
  const candidates = [process.env.TABLER_ILLUSTRATIONS_DIR, join(repoRoot, '..', 'axqdesign-illustrations')].filter((dir): dir is string => Boolean(dir))
  for (const dir of candidates) {
    if (existsSync(join(dir, 'dist', 'png', 'light'))) return dir
  }
  cloneDir = mkdtempSync(join(tmpdir(), 'axqdesign-illustrations-'))
  console.log('No local checkout found, cloning axqdesign/axqdesign-illustrations…')
  execFileSync('git', ['clone', '--depth', '1', 'https://github.com/axqdesign/axqdesign-illustrations.git', cloneDir], { stdio: 'inherit' })
  return cloneDir
}
...
try {
  ...
  for (const path of [lightDir, darkDir, packageFile]) {
    if (!existsSync(path)) throw new Error(`import-illustrations: missing ${path} — not a axqdesign-illustrations checkout?`)
  }
